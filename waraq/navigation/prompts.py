...
def rebuild_section_system() -> str:
    return (
        "أنت محلل متخصص في وثيقة معايير المحاسبة المصرية. "
        "الوثيقة مقسمة إلى أقسام رئيسية وفرعية. "
        "مهمتك كتابة 'الوصف الاسترجاعي' لكل قسم رئيسي — "
        "وهو نص مختصر يُستخدم في نظام بحث آلي ليحدد ما إذا كان هذا القسم "
        "يجيب على سؤال المستخدم. "
        "الوصف الاسترجاعي الجيد يُجيب على ثلاثة أسئلة معاً: "
        "ما المواضيع الرئيسية التي يضمّها هذا القسم؟ "
        "ما المفاهيم أو الالتزامات التي يُعرّفها؟ "
        "ما نماذج الأسئلة التي يستطيع الإجابة عنها؟ "
        "اكتب بالعربية الفصحى في فقرة واحدة من ثلاث إلى خمس جمل. "
        "لا تنسخ نص المصدر مباشرة — استخلص وصِف بأسلوبك."
    )
...
def rebuild_section_prompt(
    ...
    parts = [
        f"القسم المطلوب كتابة وصفه الاسترجاعي: {title}",
        "فيما يلي مواد المصدر لمساعدتك على فهم محتوى هذا القسم:",
    ]
    if toc_content:
        parts.append(
            f"[قائمة محتويات القسم كما وردت في الوثيقة — تُظهر الموضوعات الفرعية]\n{toc_content}"
        )
    if intro_content:
        parts.append(
            f"[نص مقدمة القسم كما ورد في الوثيقة — يشرح الهدف والنطاق]\n{intro_content}"
        )
    if child_hooks:
        hooks_text = "\n".join(f"- {h}" for h in child_hooks)
        parts.append(
            f"[الأوصاف الاسترجاعية للأقسام الفرعية المباشرة — تعكس ما تم تحليله فعلاً]\n{hooks_text}"
        )
    parts.append(
        "بناءً على مواد المصدر أعلاه، اكتب الوصف الاسترجاعي لهذا القسم. "
        "تذكر: الهدف أن يقرأه شخص يبحث عن موضوع معين فيعرف على الفور "
        "ما إذا كان هذا القسم يجيب على سؤاله أم لا."
    )
    return "\n\n".join(parts)
